...
import type {
  SerializableCourse,
  SerializableSession,
} from "@/lib/courses/published-courses";

export default function CoursesBlockView({
  data,
  courses,
}: {
  data: CoursesBlockData;
  ...
  const formatTimeRange = (startAt: string, durationMinutes: number) => {
    const start = new Date(startAt);
    const end = new Date(start.getTime() + durationMinutes * 60 * 1000);
    return `${formatBerlinTime(start)} – ${formatBerlinTime(end)}`;
  };

  const getTimeSlots = (sessions: SerializableSession[]) => {
                  ...
                  <div className="panel-body">
                    {multiWeekCourses.length > 0 ? (
                      multiWeekCourses.map((course) => {
                        ...
                        const bookingCount = course.bookingCount;
